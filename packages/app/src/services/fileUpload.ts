...
export default class FileUpload extends ApiClient {
  ...
  fileUploadControllerComplete({
    id,
    uploadId,
  }: types.FileUploadControllerCompleteParams) {
   ...
   */
  fileUploadControllerDownload(
    params: types.FileUploadControllerDownloadParams
  ) {
    const { id, ...query } = params;
    const url = `/containers/${id}/workspace/upload/download?${this.serialize(
      query
    )}`;
   ...
   */
  fileUploadControllerInitMultipart(
    params: types.FileUploadControllerInitMultipartParams1
  ) {
    const { id, ...body } = params;
