...
export default class AgentProxy extends ApiClient {
   ...
   */
  proxyControllerWithPathGet({
    path,
    agentId,
  }: types.ProxyControllerWithPathGetParams) {
   ...
   * @description 将请求透传到指定 Agent 容器的任意路径。成功时直接返回容器原始响应（不包裹统一结构）；失败时返回 `{ code, data, message }`。
   */
  proxyControllerWithPathPut({
    path,
    agentId,
  }: types.ProxyControllerWithPathPutParams) {
    const url = `/agent-proxy/${agentId}/${path}`;
    const config: DocReqConfig = { url, method: "put" };
   ...
   * @description 将请求透传到指定 Agent 容器的任意路径。成功时直接返回容器原始响应（不包裹统一结构）；失败时返回 `{ code, data, message }`。
   */
  proxyControllerWithPathPost({
    path,
    agentId,
  }: types.ProxyControllerWithPathPostParams) {
    const url = `/agent-proxy/${agentId}/${path}`;
    const config: DocReqConfig = { url, method: "post" };
   ...
   * @description 将请求透传到指定 Agent 容器的任意路径。成功时直接返回容器原始响应（不包裹统一结构）；失败时返回 `{ code, data, message }`。
   */
  proxyControllerWithPathHead({
    path,
    agentId,
  }: types.ProxyControllerWithPathHeadParams) {
    const url = `/agent-proxy/${agentId}/${path}`;
    const config: DocReqConfig = { url, method: "head" };
   ...
   * @description 将请求透传到指定 Agent 容器的任意路径。成功时直接返回容器原始响应（不包裹统一结构）；失败时返回 `{ code, data, message }`。
   */
  proxyControllerWithPathPatch({
    path,
    agentId,
  }: types.ProxyControllerWithPathPatchParams) {
    const url = `/agent-proxy/${agentId}/${path}`;
    const config: DocReqConfig = { url, method: "patch" };
   ...
   * @description 将请求透传到指定 Agent 容器的任意路径。成功时直接返回容器原始响应（不包裹统一结构）；失败时返回 `{ code, data, message }`。
   */
  proxyControllerWithPathDelete({
    path,
    agentId,
  }: types.ProxyControllerWithPathDeleteParams) {
    const url = `/agent-proxy/${agentId}/${path}`;
    const config: DocReqConfig = { url, method: "delete" };
   ...
   * @description 将请求透传到指定 Agent 容器的任意路径。成功时直接返回容器原始响应（不包裹统一结构）；失败时返回 `{ code, data, message }`。
   */
  proxyControllerWithPathOptions({
    path,
    agentId,
  }: types.ProxyControllerWithPathOptionsParams) {
    const url = `/agent-proxy/${agentId}/${path}`;
    const config: DocReqConfig = { url, method: "options" };
